...
test(`${TAGS.API} ${TAGS.PRODUCTS} should not update product with invalid token`, async ({
  productService,
}) => {
  ...
  validateResponse(updateProductResponse, STATUS_CODES.UNAUTHORIZED, false, 'Invalid access token');
});
